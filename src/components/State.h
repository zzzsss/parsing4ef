...
public:
	...
	inline int travel_lr(int i, int which, int steps);	//(steps): left:-1,-2,..., right£º1,2,...
...
inline int State::travel_lr(int i, int which, int steps)
{
	vector<int>* current_li = &nb_right;
	if(which < 0)	// which<0 means left, else right
		current_li = &nb_left;
...
inline int State::travel_spine(int i, int which, int steps)
{
	int cur = i;
	while(steps>0 && partial_heads[cur] != NOPE_YET){
		cur = partial_heads[cur];
		steps--;
	}
	return travel_lr(cur, which, steps);
}
